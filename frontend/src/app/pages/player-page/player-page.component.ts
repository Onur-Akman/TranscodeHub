...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterLink, VideoSettingsComponent],
  template: `
      ...
      <div class="player-section" *ngIf="job && (job.status === 'COMPLETED' || (job.status === 'IN_PROGRESS' && job.inputFileName === 'LIVE_STREAM'))">
        <div class="video-wrapper card">
          ...
          <app-video-settings
            ...
            (qualityChange)="onQualitySelect($event)"
            (subtitleChange)="onSubtitleSelect($event)"
            (dubChange)="onDubSelect($event)">
          </app-video-settings>
